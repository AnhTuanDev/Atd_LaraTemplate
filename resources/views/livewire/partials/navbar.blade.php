<section class="bg-primary-dark">
    <div class="w-full max-w-7xl mx-auto">
        <div x-data="{ open: false }" @click.outside="open = false"
            ...
            <nav :class="{'flex': open, 'hidden': !open}" 
                ...
                <a 
                    class="px-4 py-2 mt-2 text-md font-semibold text-primary-text capitalize md:mt-0
                ...
                <div @click.away="open = false" class="relative" x-data="{ open: false }">
                    <button @click="open = !open"
                        class="flex flex-row items-center w-full px-4 py-2 mt-2 text-md font-semibold 
                    ...
                    <!-- dropdown menu -->
                    <div x-show="open" 
                         x-transition.enter.scale.80.origin.top.right.duration.200ms
                         x-transition.leave.scale.80.origin.top.right.duration.50ms
                        ...
                        <div class="px-2 py-2 bg-white rounded-md shadow">
                            @foreach($gioiThieu as $menu)
                                <a class="block px-4 py-2 mt-2 capitalize text-md font-semibold text-primary-text
                ...
                <div class="inline-flex items-center gap-2 list-none lg:ml-auto w-full sm:max-w-lg">
                    <form action="" 
                          method="post"
                          name="revue-form" target="_blank" class="
                                  p-1 transition duration-500 ease-in-out transform
                                  bg-primary-light rounded-xl w-full sm:max-w-lg sm:flex">

                        <div class="flex-1 min-w-0 block md:flex md:items-center">
                            <label for="search" class="sr-only">Search</label>
                            <input id="cta-email" type="text" class="
                                       block w-full px-5 py-1 md:py-2 text-base text-primary-text placeholder-primary-dark transition
                                       ...
                                       focus:ring-white focus:ring-offset-2 focus:ring-offset-primary-dark " 
                        ...
                        <div class="mt-4 sm:mt-0 sm:ml-3 revue-form-actions">
                            <button type="submit" value="Subscribe" name="member[subscribe]" id="member_submit" class="
                                          block w-full px-5 py-1 md:py-2 text-base font-medium
                                          ...
                                          focus:ring-white focus:ring-offset-2 focus:ring-offset-primary-dark sm:px-10
                                          "> Tìm Kiếm </button>
                        </div>
                    </form>
                </div>
    ...
    <div class="bg-primary-main">
        <div x-data="{ subMenu: null }" 
             @click.outside="subMenu = null"
             @mouseover.away="subMenu = false"
             class="max-w-7xl mx-auto px-6 py-2 border-b border-priamry-dark">

            <div class="flex flex-wrap justify-start items-center capitalize -ml-4">
                ...
                @foreach($mainMenu as $menu) 
                    ...
                    <div class="relative">
                        ...
                        <div @mouseover.prevent="subMenu = {{ $menu->id }}" 
                         @click.prevent="subMenu = {{ $menu->id }}" 
                         class="main-menu-hover flex px-4 py-2 md:py-3 text-lg font-semibold md:text-xl">
                         {{ $menu->name}}
                        </div>

                            <div x-show="subMenu == {{ $menu->id }}"
                                 x-transition.enter.scale.80.origin.top.left.duration.200ms
                                 x-transition.leave.scale.80.origin.top.left.opacity.duration.50ms
                                 class="absolute w-64 p-6 left-0 top-0 mt-[2.5rem] md:mt-[4rem] on-top border-b-4 border-secondary-main flex flex-col 
                                        bg-primary-main space-y-4 shadow-lg rounded-b-lg"
                                 style="display: none">

                                        @foreach($menu->child as $child) 
                                        <a href="{{ route('home') }}" class="hover:text-secondary-dark active:text-indigo-500 duration-200 hover:font-bold text-md md:text-lg">
                                            {{ $child->name }}
                                        </a>
                                        @endforeach
                            </div>
