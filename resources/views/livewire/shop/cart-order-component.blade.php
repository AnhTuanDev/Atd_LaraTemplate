...
<section class="content">
    ...
    <div class="w-full max-w-7xl mx-auto p-5 md:flex md:space-x-5">
                            ...
                            <div class="w-full flex items-center justify-between">
                                <div class="flex">
                                    ...
                                    <div> @php $pro = App\Models\Shop\Product::findOrFail($item->id); @endphp
                                        <a @click="$store.shop.overflow = false"
                                            href="{{ route('shop.product.show', $pro) }}">
        ...
        <!-- Cart action -->
        <form action="{{ route('shop.cart.checkout', $orderId) }}"
            ...
            @if($cartCount)
                <div class="rounded-md w-full border-primary-dark mt-4 md:mt-0 p-4 border-2 border">
                    ...
                    <div class="flex items-center mt-4 text-gray-700">
                        <x-orchid-icon path="bell" />
                        <label class="ml-2 text-sm leading-none">TPHCM free ship với đơn hàng trên 250.000 đ </label>
                    </div>
                    <div class="flex items-center mt-4 text-gray-700">
                        <x-orchid-icon path="bell" />
                        <label class="ml-2 text-sm leading-none">Ngoài TPHCM free ship với đơn hàng trên 400.000 đ</label>
                    </div>
                    ...
                    <!-- Cart ghi chu -->
                        ...
                        @trix(\App\Shop\Oder::class, 'content', [ 'hideTools' => ['file-tools', 'block-tools'] ] )
                    </div>
                </div>
                <!-- Thanh toan -->
